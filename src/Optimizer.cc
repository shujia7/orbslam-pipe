...
namespace ORB_SLAM2
{
Eigen::Matrix<double, 5, 1> Optimizer::mCyVar = Eigen::Matrix<double, 5, 1>::Ones();

bool Optimizer::mbCyVar = 0;
std::list<MapPoint *> Optimizer::mlVpointCloud;
std::unordered_set<unsigned long> Optimizer::musPointId;
std::unordered_map<MapPoint *, unsigned long> Optimizer::mumMapPoint2Id;
...
bool Optimizer::EstimateCylinder(MapCylinder* pcy, std::list<MapPoint *>& MapPoints){
    vector<MapPoint *> vMapPoints(MapPoints.begin(), MapPoints.end());
    std::vector<Eigen::Vector3d> points;
    std::unordered_map<int , int> point_mappoint;

    points.reserve(vMapPoints.size());
    for(size_t i = 0; i < vMapPoints.size(); i++){
        MapPoint *mp = vMapPoints[i];
        if(mp){
            points.emplace_back(Converter::toVector3d(mp->GetWorldPos()));
            point_mappoint.insert({points.size() - 1, i});
        }
    }
    ...
    vbPoints.reserve(points.size());

  
    for (size_t j = 0; j < points.size(); j++) {
        
        g2o::VertexSBAPointXYZ *vPoint = new g2o::VertexSBAPointXYZ();
        vPoint->setEstimate(points[j]);
        vPoint->setId(j + 3);
    ...
    int inlier = points.size();
    double merror = 0;
    while(standarddeviation > threshold){
    ...
    for(int i = 0; i < points.size(); i++){
        ...
        if(vbPoints[i] == true){
            vMapPoints[point_mappoint[i]]->SetCylinder(pcy);
        }
...
{    
    ...
    // 没加入的点加入mlVpointCloud
    // for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++){
    //     MapPoint* pMP = *lit;
    //     if(pMP){
    //         if(musPointId.find(pMP->mnId) == musPointId.end() && pMP->nObs > 3){
    //             cv::Mat pos = pMP->GetWorldPos();
    //             mlVpointCloud.push_back(pMP);
    //             mumMapPoint2Id.insert({(pMP), pMP->mnId});
    //             musPointId.insert(pMP->mnId);
    //         }
    //     }
